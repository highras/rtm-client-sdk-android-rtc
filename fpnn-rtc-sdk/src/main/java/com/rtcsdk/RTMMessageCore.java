...
class RTMMessageCore extends RTMCore {
    ...
    private ModifyTimeStruct sendMsgSync(long id, int mtype, Object message, String attrs,  MessageTypes type) {
    ...
    private Quest genGetMessageQuest(long id, boolean desc, int count, long beginMsec, long endMsec, long lastId, List<Integer> mtypes, MessageTypes type)
    {
    ...
    void getHistoryMessages(final IRTMCallback<HistoryMessageResult> callback, final long id, boolean desc, int count, long beginMsec, long endMsec, long lastId, List<Integer> mtypes,  final MessageTypes type) {
        ...
        sendQuest(quest, new FunctionalAnswerCallback() {
            ...
            public void onAnswer(Answer answer, int errorCode) {
                ...
                if (errorCode == okRet) {
                    ...
                    if (type == MessageTypes.P2PMessage)
    ...
    HistoryMessageResult getHistoryMessages(final long id, boolean desc, int count, long beginMsec, long endMsec, long lastId, List<Integer> mtypes,  MessageTypes type){
        Quest quest = genGetMessageQuest(id, desc, count, beginMsec, endMsec, lastId, mtypes, type);
        Answer answer = sendQuest(quest);
...
//        RTMAnswer result = genRTMAnswer(answer);
    ...
    ModifyTimeStruct internalSendMessage(long toid, int mtype, Object message, String attrs, MessageTypes msgType) {
        if (mtype <= MessageType.NORMALFILE)
    ...
    ModifyTimeStruct internalSendChat(long toid, int mtype, Object message, String attrs, MessageTypes msgType) {
        return sendMsgSync(toid, mtype, message, attrs,  msgType);
    }

    ArrayList<ConversationInfo> parseHis( ArrayList<List<Object>> msgs, ArrayList<Long> conversations, ArrayList<Integer> unreadNums) {
        ArrayList<ConversationInfo> rets = new ArrayList<>();

        for (int i = 0; i < conversations.size(); i++) {
            ConversationInfo conversationInfo = new ConversationInfo();
            conversationInfo.toId = conversations.get(i);
            ...
            if (tmp.fromUid == 1) {
                tmp.fromUid = getUid();
                tmp.toId = conversationInfo.toId;
            } else {
                tmp.fromUid = conversationInfo.toId;
                tmp.toId = getUid();
            }
            tmp.messageType = (byte) rtmUtils.wantInt(value.get(2));
            tmp.messageId = rtmUtils.wantLong(value.get(3));
            ...
            try {
                ...
                conversationInfo.lastHistortMessage = tmp;
            } catch (Exception ex) {
                errorRecorder.recordError("buildHistoryMessageResult parse json failed " + ex.getMessage());
                continue;
            }
